...
def write_chunkindex_to_repo_cache(
    ...
    # for now, we don't want to serialize the flags or the size, just the keys (chunk IDs) + pack_id:
...
def build_chunkindex_from_repo(repository, *, disable_caches=False, cache_immediately=False):
    ...
    for id, stored_size in repo_lister(repository, limit=LIST_SCAN_LIMIT):
        num_chunks += 1
        chunks[id] = ChunkIndexEntry(flags=ChunkIndex.F_USED, size=0, pack_id=id, pack_offset=0, pack_size=stored_size)
