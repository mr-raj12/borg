...
def superseded_gap_ranges(reader, chunks, pack_id, obj_ranges, pack_size, *, validate):
    ...
    obj_ranges: the offset-ordered (obj_offset, obj_size) ranges of the pack's indexed objects,
        non-overlapping and within the pack. The gaps are the byte ranges between and after them.
    pack_size: size of the pack in bytes.
    validate: validate(chunk_id, obj) -> bool, see repoobj.object_validator. obj is the object's
    ...
    for gstart, gend in gaps:
        ...
        while offset < gend:
            ...
            if hdr is None:
                logger.warning(
                    f"pack {pack_hex}: {problem} at offset {offset} in a gap, "
                    f"keeping the remaining {gend - offset} bytes of the gap."
                )
                break
            obj_size = hdr_size + hdr.meta_size + hdr.data_size
            ...
            if entry is not None and (entry.pack_id != pack_id or entry.obj_offset != offset):
                ...
                if problem is None:
                    ...
                    logger.warning(f"pack {pack_hex}: {problem} at offset {offset} in a gap, keeping its bytes.")
...
class Repository:
    ...
    def check(self, repair=False, max_duration=0, max_age=0, repo_only=False, validate=None):
        ...
        MAC (a tag that only the key holder can compute), so the sha256 check detects accidental
        corruption but not tampering. The rebuild checks each object with validate, see below, refs
        #9901, #10026. If any pack is corrupt the index is left unchanged, refs #8572, #10026. Pack ids
        found corrupt are kept in cache/checked-packs, refs #9696.
        ...
        validate: validate(chunk_id, obj) -> bool, True if obj (an object's header and metadata slot) is
        the repo object with id chunk_id, see repoobj.object_validator. The index rebuild calls it for
        every object header it reads and does not index an object it rejects; each pack byte range it
        skips counts as one error. In repair mode these errors fail a repository-only run only, as a
        corrupt pack does. None: the rebuild indexes every object whose header parses, and raises
        CorruptPack at a header that does not.
        """
        ...
        drops = 0  # number of pack byte ranges the index rebuild skipped, as counted by on_drop
        packs_scanned = False
        ...
        if drops:
            logger.error(
                f"The index rebuild skipped {drops} pack byte range(s) holding objects that failed validation."
            )
        ...
        if not problems:
            ...
            # lacks, so it only warns here.
            log = logger.error if repo_only else logger.warning
            log(f"{done} {mode} repository check, index rebuilt without objects that failed validation{so_far}.")
            ...
            logger.error(f"{done} {mode} repository check, errors found{so_far}.")
        # in repair mode a corrupt index left unrebuilt is a failure; a corrupt or missing pack, or pack
        # bytes the rebuild skipped, fail only a repository-only run, while a full check defers them to
        # the archives phase.
        if repair:
    ...
    def compact_pack(self, pack_id, *, keep_ids: set, drop_ids: set, validate, chunks=None):
        ...
        stale when the old pack is deleted. Gaps (byte ranges no index entry covers) are copied into
        the new pack, except the superseded duplicates superseded_gap_ranges reports. An overlap
        between listed objects, or an object claiming to end past the pack file, means index
        corruption and raises IntegrityError.
    ...
    def transform_pack(self, pack_id, ids, transform, *, validate, chunks=None, before_change=None):
        ...
        superseded_gap_ranges reports. An overlap between indexed objects, or an object claiming to
        end past the pack file, means index corruption and raises IntegrityError, before anything is
        written.
